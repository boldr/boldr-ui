...
import getDomFromString from '../../../internal/jest/utils/get-dom-from-string';
import getStylesFromDomNode
  from '../../../internal/jest/utils/get-styles-from-dom-node';
...
const getShallowNode = props => {
  ...
  return shallow(<SpeedDial {...Object.assign({}, defaultProps, props)} />, {
    context,
  });
};
...
describe('<SpeedDial />', () => {
    ...
    it('with toolbox left', () => {
      ...
      ReactDOM.render(
        <MuiThemeProvider>
          <SpeedDial
            positionH="left"
            toolbox={{ className: 'toolbox', height: 56 }}
          >
            <BubbleList />
          </SpeedDial>
        </MuiThemeProvider>,
  ...
  describe('prop positionH', () => {
    it('top => button wrapper has top=16px', () => {
      ...
      const button = getStylesFromDomNode(
        getDomFromString(div.innerHTML).getElementsByTagName('button')[0]
  ...
  describe('style props', () => {
    ...
    it('prop styleBackdrop is color:red', () => {
      ...
      expect(getStylesFromShallowNode(wrapper.find('.backdrop')).color).toEqual(
        'red',
      );
    });
  });
  ...
  describe('states', () => {
    it('state closing', () => {
      ...
      const attrClass = getDomFromString(
        wrapper.find('.morph-btn').html(),
      ).getAttribute('class');
