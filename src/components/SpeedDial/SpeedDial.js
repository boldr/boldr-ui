...
import BubbleListItem from './BubbleListItem/BubbleListItem';
import propTypes from './SpeedDial.propTypes';
import defaultProps from './SpeedDial.defaultProps';
import getStyles from './SpeedDial.styles';
import { getCssKeyFrames, getCssKeyFramesClosing } from './SpeedDial.keyframes';
...
class SpeedDial extends React.Component {
  ...
  constructor(props, context) {
    ...
    if (this.isControlled && typeof props.onChange !== 'function') {
      ...
      console.error(
        'Warning: Failed SpeedDial propType: You provided a `isOpen` prop to a SpeedDial without an `onChange` handler',
      );
    }
  }
  ...
  getActionButtonStyles() {
    try {
      return Object.assign(
        {},
        this.getActionButton().refs.enhancedButton.style,
      );
    } catch (err) {
      return {};
    }
  ...
  getStylesBackdrop() {
    ...
    const styles = this.styles;
    const stylesLink = isOpen
      ? styles.backdrop.main
      : styles.backdrop.invisible;
  ...
  renderIcon() {
    ...
    return [
      React.cloneElement(icon, {
        key: '0',
        style: isOpen
          ? this.styles.iconClosed.invisible
          : this.styles.iconClosed.main,
      }),
      React.cloneElement(iconOpen, {
        key: '1',
        style: isOpen
          ? this.styles.iconOpen.main
          : this.styles.iconOpen.invisible,
      }),
    ];
  }
  ...
  renderBackdrop() {
    ...
    const { isOpen } = this.state;
    const styles = this.styles;
    const stylesWrap = isOpen
      ? styles.backdropWrap.main
      : styles.backdropWrap.invisible;
  ...
  renderCssKeyframes() {
    ...
    return (
      <style>
        ...
        {getCssKeyFramesClosing(
          keyFrameClosingClassName,
          this.instanceKey,
          options,
        )}
      </style>
