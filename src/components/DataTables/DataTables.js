...
function getStyles(props, context) {
  const { baseTheme: { palette }, table, tableHeaderColumn } = context.muiTheme;
...
class DataTables extends Component {
  ...
  handleCellClick = (rowIndex, columnIndex, event) => {
    ...
    if (onCellClick && !selectable) {
      const adjustedColumnIndex = this.props.showCheckboxes
        ? columnIndex
        : columnIndex - 1;
      onCellClick(
        rowIndex,
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Toolbar
          style={Object.assign({}, styles.footerToolbar, footerToolbarStyle)}
        >
