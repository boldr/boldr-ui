...
class DataTablesHeaderToolbar extends Component {
  ...
  render() {
    ...
    const { mode, filterValue } = this.state;
    ...
    if (mode === 'default') {
      contentNode = (
        <ToolbarTitle
          style={Object.assign({}, styles.toolbarTitle, titleStyle)}
          text={title}
        />
      );
    } else if (mode === 'filter') {
    ...
    if (toolbarIconRight) {
      if (toolbarIconRight.length) {
        toolbarIconRight.map((toolbarIcon, i) => {
          toolbarIconRightChildren.push(
            React.cloneElement(toolbarIcon, {
              style: Object.assign(styles.headerToolbarIconButton, styles.icon),
              key: i,
            }),
          );
        ...
        toolbarIconRightChildren.push(
          React.cloneElement(toolbarIconRight, {
            style: Object.assign(styles.headerToolbarIconButton, styles.icon),
            key: 1,
          }),
        );
    ...
    return (
      <Toolbar style={styles.headerToolbar}>
        ...
        <ToolbarGroup>
          ...
          >
            <FilterListIcon color={mode === 'filter' ? blue500 : ''} />
          </IconButton>
