  ...
  return (
    <RouterLink {...rest} to={to} className={finalClassName}>
      <div className={style.cardImageWrapper}>
        {asset({ className: style.cardImage })}
      </div>
      <div className={style.contents}>
        <Headline type={headlineType}>{title}</Headline>
        {children ? children : <Paragraph>{text}</Paragraph>}
        {actions
...
Card.propTypes = {
  ...
  /**
	 * The title of the page to be teased, optional since you can also pass in raw children.
	 */
